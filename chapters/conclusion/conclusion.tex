...
This work set out to investigate the feasibility and performance of a fully unconstrained Cartesian mesh implementation of nuclear Density Functional Theory based on Skyrme EDFs. The central objective was to assess whether the Generalised Conjugate Gradient eigensolver could provide a robust and efficient alternative to conventional iterative diagonalisation methods for the minimisation of nuclear energy functionals.

\section*{Aim and findings}
...
Beyond these validation tests, the code was used to explore clustering phenomena in the ground state of $^{20}$Ne, as well as the structure of the near-drip line nuclei $^{42}$Si and $^{28}$S. These applications highlight the flexibility of the method in handling strongly deformed, correlated, and weakly bound configurations.

Overall, the GCG method proved to be a robust and promising tool for EDF calculations, particularly in the frame of unconstrained Cartesian meshes, which require the diagonalisation of large-scale matrices. Its scalability and iterative nature make it a suitable candidate for future large-scale studies, not only of nuclear deformation, fission pathways, and collective excitations but in other areas of science as well.

\section*{Future developments}

The implementation presented here establishes a solid foundation but also leads the way for further improvement. These may be grouped into two broad categories: numerical developments and physics extensions.

\subsection*{Numerical improvements}

While the GCG eigensolver enables unconstrained calculations, further work is required to optimise performance and scalability. The most relevant numerical improvements include:
\begin{itemize}
...
Finally, when physical symmetries are known to be conserved -- such as axiality, parity, or certain reflection symmetries -- the computational domain can be restricted to selected octants of the Cartesian mesh. Implementing symmetry-adapted reductions would significantly decrease computational cost while preserving accuracy.
...
\begin{itemize}
    \item incorporating pairing correlations through a BCS or full HFB framework with an effective pairing interaction, enabling the description of superfluid even-even systems;
    ...
    \item implementing cranking calculations to access rotational excitations and the spectra of deformed nuclei.
\end{itemize}
...
Although many extensions remain to be implemented, the results presented in this thesis already demonstrate the potential of an unconstrained mesh DFT solver based on an iterative eigensolver approach. The ability to perform three-dimensional, symmetry-unrestricted calculations with modest computational resources opens promising prospects for applications across nuclear structure and fission theory.
