...
Overall, the GCG method proved to be an efficient and promising tool for DFT calculations, particularly in the frame of unconstrained Cartesian meshes, which require the diagonalisation of large-scale matrices. The efficiency and precision of GCG make it a suitable candidate for future unconstrained studies, not only for Skyrme functionals, but ab-initio methods and physics applications as well.
...
\begin{itemize}
    \item incorporating pairing correlations through a BCS or full HFB framework with an effective pairing interaction, enabling the description of superfluid systems;
...
Although many extensions remain to be implemented, the results presented in this thesis already demonstrate the potential of an unconstrained mesh DFT solver based on GCG. The ability to perform three-dimensional, symmetry-unrestricted calculations with modest computational resources opens promising prospects for applications across nuclear structure and fission theory.
