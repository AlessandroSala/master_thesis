...
\end{equation}
where $R_0$ is the nuclear radius, which can be parametrized as $R_0\approx 1.2A^{1/3}$, and $a$ is the diffusivity, whose value determines how sharp the density drops from its saturation value $\approx \rho_0$ to $\approx 0$. 
...
\begin{figure}[h]
    ...
    \caption{Graphical representation of the harmonic oscillator shells, together with the spin-orbit coupling. Shells whose total angular momentum is $j=l+1/2$ are lowered in energy, viceversa for total angular momentum $j=l-1/2$. Figure adapted from \cite{Colo_2021_NuclearPhysics}.}
