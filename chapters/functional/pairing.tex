...
The ansatz of BCS is that nucleons are paired in states whose total angular momentum is zero, so that the wavefunction of pair is $\ket{JM}=\ket{00}$ and reads
\begin{equation}
    \label{eq:zero_mom}
...
\begin{equation}
    \kappa = UV^\dagger,
\end{equation}
while the density matrix, defined in \eqref{eq:density_op}, in matrix form reads
\begin{equation}
    \quad \rho = V^*V^T.
\end{equation}
