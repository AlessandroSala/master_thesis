...
An alternative approach can be used, as in a stationary point $\delta E = 0$, the single particle eigenvalue Equation~\eqref{eq:spe_ks} stands true, summarised as 
\begin{align}
    \label{eq:simple_spe}
...
\end{align}
and given the total energy of the system from Equation~\eqref{eq:total_energy_HF}
\begin{align}
...
\paragraph{Sidenote}
The actual functional, including the Coulomb exchange term, has different $\rho$ terms, which can be summarised as
\begin{equation*}
...
Since Equation~\eqref{eq:hf_energy} is valid only for $\delta E = 0$, it is useful to check its equivalence with the integrated energy at convergence, so one can be sure to actually be in a stationary point of the EDF.