...
The whole Hartree--Fock framework presented up to this point, has been implemented using the C++ language \cite{stroustrup1986overview} and the Eigen library \cite{eigenweb}, which implements linear algebra operations through low level routines such as $\texttt{LAPACK}$ and $\texttt{BLAS}$. In Figure~\ref{fig:pseudocode}, the schematics of the program structure is reported.
\begin{figure}[h]
\begin{forest}
  for tree={
    draw,
