...
It was realised early on that the only way to obtain realistic results from this approach was to use density dependent forces, which somewhat account for the important many-body nuclear forces. The historical development of this realization has been to use the Hartree--Fock expectation value of an effective interaction as a starting point for the design of an Energy Density Functional (EDF), which can be used to do nuclear structure calculations using Density Functional Theory.

In particular, the minimisation of these EDFs has been done using two numerical approaches: (a) \textit{basis expansion methods}, which represent single-particle states on truncated complete bases, mainly harmonic oscillator bases, 
...
The HO basis, however efficient, introduces inherent limitations.
...
\begin{itemize}
    ...
    \item attempt to produce original results that specifically require an unconstrained implementation of this kind, and establish the advance brought to the field by this work.
\end{itemize}
